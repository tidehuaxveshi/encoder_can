...
/*
 * Map value from outter_lower to outter_uppper in the range in_lower to in_upper.
 * If value is outside of the range it will be less truncated to the closest marginal value.
 * Angle units: Radians.
 */
float domain_mapping(float in_lower, float in_upper, float out_lower, float out_uppper, float value)
{
    if (value < in_lower)
    {
        return out_lower;
    }
    else if (value > in_upper)
    {
        return out_uppper;
    }
    else
    {
        return (value - in_lower) / (in_upper - in_lower) * (out_uppper - out_lower) + out_lower;
    }
}
/*
 * Minimun angle calculation
 */
float minimun_angle_cal_radian(float current_angle, float target_angle)
{
    float diff = angle_difference_rad(target_angle, current_angle);
    if (diff > M_PI)
    {
        return current_angle + diff - 2 * M_PI;
    }
    else if (diff <= M_PI && diff > 0.0)
    {
...
float minimun_angle_cal_degree(float current_angle, float target_angle)
{
    float diff = angle_difference_degree(target_angle, current_angle);
    if (diff > 180.0)
    {
        return current_angle + diff - 2 * 180.0;
    }
 ...
 * angle. Angle units: Radians.
 */
float map_angle_rad(float angle, float min, float max)
{
    if (max == min)
    {
        return -1;
    }
    // rotate min and max to 0 and uppper
    float uppper = norm_angle_degree(max - min);

    angle = norm_angle_degree(angle - min);

    if (angle > (uppper * 1.5))
    {
        return 0;
    }
    else if (angle < 1.5 * uppper && angle > uppper)
    {
        return uppper;
    }
    else
...
/**
 * Get the difference between two angles. Will always be between 0 and +360 degrees.
 * @param angle1
 * The first angle
 * @param angle2
 * The second angle
 ...
 */
float angle_difference_degree(float angle1, float angle2)
{
    float difference = angle1 - angle2;
    return norm_angle_degree(difference);
}

/**
 * Get the difference between two angles. Will always be between 0 and +2pi radians.
 * @param angle1
...
{
    float difference = angle1 - angle2;
    return norm_angle_rad(difference);
}
...
float utils_batt_liion_norm_v_to_capacity(float norm_v)
{
    ...
    norm_v = utils_truncate_number(norm_v, 0.0, 1.0);
