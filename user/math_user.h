...
#define BOUND(min, max, value) ((value) > (max) ? (max) : ((value) < (min) ? (min) : (value)))
...
#define NORM3_f(x, y, z) (sqrtf(SQ(x) + SQ(y) + SQ(z)))
 ...
 */
static inline float norm_angle_degree(float angle)
{
 ...
 * The angle to normalize.
 */
static inline float norm_angle_degree_about_zero(float angle)
{
 ...
 */
static inline float norm_angle_rad(float angle)
{
    while (angle < 0)
    {
        angle += 2.0 * M_PI;
 ...
 * WARNING: Don't use too large angles.
 */
static inline float norm_angle_rad_about_zero(float angle)
{
    angle = fmodf(angle, M_PI);
    return angle;
}

static inline float utils_truncate_number(float number, float min, float max)
{
    if (number > max)
    {
        number = max;
